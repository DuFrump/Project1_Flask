...
try:
    with open(stopwords_path, 'r', encoding='utf-8') as f:
        ...
        print(f"가공된 불용어 {len(my_stopwords)}개를 파일에서 성공적으로 불러왔습니다.")
except FileNotFoundError:
    print(f"경고: 불용어 파일('{stopwords_path}')을 찾을 수 없습니다.")
    my_stopwords = []
...
keyword_weights = {
    '스포츠': ['감독', '경기', '구단', '농구', '득점', '리그', '선수', '승리', '야구', '운동', '월드컵', '올림픽', '응원', '이적', '챔피언', '축구', '훈련'],
    '경제/제테크': ['경제', '금리', '나스닥', '대출', '매도', '매수', '부동산', '세금', '수익', '시장', '예금', '적금', '주가', '주식', '증시', '채권', '투자', '펀드', '환율', 'ETF', '코스피', '기술주'],
    '기술/IT': ['네트워크', '데이터', '드라이버', '반도체', '서버', '소프트웨어', '스마트폰', '알고리즘', '어플', '업데이트', '인공지능', '코딩', '프로그래밍', '하드웨어', 'AI', 'CPU', 'GPU', '해킹', '해커', '프로그래머', '개발자', '드라이버', '컴퓨터', '데스크탑', '노트북'],
    '영화/미디어': ['감독', '개봉', '극장', '넷플릭스', '드라마', '리뷰', '배우', '시나리오', '애니메이션', '연기', '예고편', '영화', '작품', '주연', '촬영', 'OTT', '구독료', '다큐멘터리', '유튜브', '영상', '동영상'],
...
final_model.fit(X_train_resampled, y_train_resampled, verbose=2)
...
y_train_pred_encoded = final_model.predict(X_train_resampled)
...
predicted_encoded = final_model.predict(samples_vec)
...
new_samples = [
    ...
    "퇴근하고 동네 공원에서 잠깐 바람 쐬는 게 요즘 내 유일한 행복이야.", "머신러닝 모델의 과적합 문제를 해결하기 위한 수학적 원리가 궁금하다.",
...
new_predicted_encoded = final_model.predict(new_samples_vec)
...
# -------------------------------------------------------------------
print("\n학습된 모델과 도구들을 저장합니다...")
joblib.dump(final_model, './Project1/models/XGBoost/korean_topic_model.pkl')
